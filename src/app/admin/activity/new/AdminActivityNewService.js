 ...
 * New admin activity
 */
export class AdminActivityNewService {
  constructor(GcResource, TriggerDownloadService) {
    ...
    this.displayData = {
      ...
      // Deduction error
      deductionError: "",
      // Denial/remittance email type
      emailType: 'remittance'
    };
  }
  ...
  /**
   * Returns an email subject, given an email type
   *
   * @param {String} type
   * @return {String}
   */
  getEmailSubjects(type) {
    switch (type) {
      case 'denial':
        return 'Denials';
      case 'chargeback':
        return 'Chargeback Notification';
      case 'remittance':
        return 'ACH Payment';
      case 'exception':
        return 'Exceptions';
  ...
  /**
   * Sends an email to the specified company's bookkeeping email addresses
   *
   * @param {String} type
   * @param {String} companyId
   * @param {String} emailBody
   */
  sendAccountingEmail(type, companyId, emailBody) {
    const subject = this.getEmailSubjects(type);

    return Resource.get(this).resource('Admin:sendAccountingEmail', {companyId, emailBody, emailSubject: subject});
  }
}
