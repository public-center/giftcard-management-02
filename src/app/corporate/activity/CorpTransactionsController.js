 ...
 */
export class CorpTransactionsController {
  constructor(CorpActivityService, $scope, $timeout, $state, alternativeGcmgr) {
    'ngInject';
    Service.set(this, CorpActivityService);
    scope.set(this, $scope);
    timeout.set(this, $timeout);
    datePurchased.set(this, null);
    state.set(this, $state);
    // On denials, rather than activity
    const stateName = $state.current.name;
    this.isDenials = stateName === 'main.corporate.denials' || stateName === 'main.corporate.customer-denials';
    // Reference displayData
    this.displayData = CorpActivityService.displayData;
    this.safeData = CorpActivityService.safeData;
    $scope.displayData = {
      safe: [],
      display: [],
  ...
  getActivityData(query) {
    return Service.get(this).getAll(query, this.isDenials, state.get(this).params.customerId, true)
   ...
   */
  setValue(params) {
    Service.get(this).setInventoryValue(params.row, params.type, true);
  }
  ...
  /**
   * Download CSV of currently displayed records
   */
  downloadCsvSheet() {
    // Get CSV
    return Service.get(this).downloadCsvSheet(this.tableState, this.isDenials);
  }
}
